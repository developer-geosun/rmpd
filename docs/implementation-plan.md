...
|-----------|---------|--------|
| P1 | RMPD406 — проверка GPS | ✅ mock-провайдер, API + UI |
| P2 | Пакетная загрузка CMR | ✅ `POST /api/v1/cmr/batch` |
| P3 | Справочник контрагентов с автоподсказкой из CMR | ✅ suggestions API + UI |
| P4 | Агентская модель (idSiscROP) | ✅ credential + GetNextDocumentSisc |
| P5 | Интеграция GPS-провайдеров | ⚠️ mock + e-TOLL stub (`GPS_PROVIDER=etoll`) |
