...
public final class SeapSoapEnvelopeBuilder {
    ...
    public static String buildGetNextDocumentSisc(
            String username,
            WsSecurityPasswordDigest.DigestResult digest,
            String messageId,
            String targetSystem,
            SiscContext context) {
        StringBuilder siscFilters = new StringBuilder();
        if (context.idSiscRop() != null && !context.idSiscRop().isBlank()) {
            siscFilters.append("      <idSiscROP>").append(escapeXml(context.idSiscRop())).append("</idSiscROP>\n");
        }
        if (context.idSiscRof() != null && !context.idSiscRof().isBlank()) {
            siscFilters.append("      <idSiscROF>").append(escapeXml(context.idSiscRof())).append("</idSiscROF>\n");
        }
        if (context.idSiscP() != null && !context.idSiscP().isBlank()) {
            siscFilters.append("      <idSiscP>").append(escapeXml(context.idSiscP())).append("</idSiscP>\n");
        }
        return """
                <?xml version="1.0" encoding="UTF-8"?>
                <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"
                  ...
                  <soapenv:Body>
                    <doc:GetNextDocumentSiscRequest>
                      <targetSystem>%s</targetSystem>
                %s    </doc:GetNextDocumentSiscRequest>
                  </soapenv:Body>
                </soapenv:Envelope>
                """.formatted(
                messageId,
